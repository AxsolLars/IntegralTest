...
CREATE OR REPLACE PROCEDURE powermeter_integrate()
RETURNS STRING
LANGUAGE PYTHON
...
def run(session: Session) -> str:
    ...
    SELECT 
        CAST(timestamp AS DATE) AS day,
        timestamp,
        POWER_REAL_SUM_L1_L2_L3 AS value
    ...
    for row in rows:
        clustered[row["DAY"]].append({"TIMESTAMP": row["TIMESTAMP"], "VALUE": row["VALUE"]})
    ...
    for day in clustered.keys():
        ...
        if rows_to_insert:
            ...
            df.write.save_as_table("power_export_sum", mode="append")
            session.sql("""
